...
**Goal:** Build `cove`, a dependency-free Go CLI that templates+packs an sbx "kit" and manages sandboxes via `build`, `create`, `run`, and `delete`.
...
Run: `go test ./internal/sbx/`
Expected: PASS (`ok  github.com/aethons-tools/cove/internal/sbx`).
...
import (
	...
	"github.com/aethons-tools/cove/internal/runner"
	"github.com/aethons-tools/cove/internal/sbx"
)
...
```go
// Command cove customizes a sbx kit for the local machine and manages
...
const usage = `cove — customize and run sbx sandboxes

Usage:
  cove build [kitdir]
  cove create <name> <kitdir> [volume...]
  cove run <name>
  cove delete <name>
...
func run(argv []string, r runner.Runner, lookup func(string) (string, bool), lookPath func(string) (string, error), stdout, stderr io.Writer) int {
	...
	if !dryRun {
		if _, err := lookPath("sbx"); err != nil {
			fmt.Fprintln(stderr, "cove: sbx not found on PATH")
			return 1
		}
	}
	...
	case "build":
		if len(rest) > 1 {
			fmt.Fprintln(stderr, "cove: build takes at most one kitdir")
			return 2
		}
		kitDir := "."
		if len(rest) == 1 {
	...
	case "create":
		if len(rest) < 2 {
			fmt.Fprintln(stderr, "cove: create requires <name> <kitdir>")
			return 2
		}
		err = kit.Create(rest[0], rest[1], rest[2:], r, lookup, opts)
	case "run":
		if len(rest) != 1 {
			fmt.Fprintln(stderr, "cove: run requires <name>")
			return 2
		}
		err = dispatch(r, opts, sbx.Run(rest[0]))
	case "delete":
		if len(rest) != 1 {
			fmt.Fprintln(stderr, "cove: delete requires <name>")
			return 2
		}
...
Run: `go vet ./... && go build -o at-cove .`
Expected: no output from vet; an `cove` binary is produced.

- [ ] **Step 6: Manual dry-run smoke test**

```bash
mkdir -p /tmp/cove-kit/bin
printf 'user=${USER}\n' > /tmp/cove-kit/config.txt
USER=alice ./at-cove create box /tmp/cove-kit --dry-run
```
...
would run: sbx kit pack /tmp/cove-kit/.build/kit -o /tmp/cove-kit/.build/kit.zip
would run: sbx run --name box --kit /tmp/cove-kit/.build/kit.zip claude .
```

Also confirm no-args usage and unknown-command both exit non-zero:

```bash
./at-cove; echo "exit=$?"          # prints usage, exit=2
./at-cove bogus; echo "exit=$?"    # prints unknown command, exit=2
```
...
git commit -m "feat: add cove CLI dispatcher with dry-run and exit-code propagation"
```
