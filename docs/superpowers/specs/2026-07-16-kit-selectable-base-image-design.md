...
2. **Base resolution + provenance gate — dual-path, additive.** *(Go + runner seam; hermetic + integration.)* The resolver (build kit `image/Dockerfile` by digest → `image.base` → default `cove-base-image@Blessed()[0]`); the `diff_ids` prefix check against `basedigest.Blessed()` (any entry, per-arch); `--allow-unverified-base` → warn (on the current build-triggering commands — interim per [COV-38](https://linear.app/aethons-tools/issue/COV-38)). Introduces the `FROM ${BASE}` ARG defaulting to the blessed digest, so the produced image is unchanged. **Dual-path so legacy kits stay green:** if `image/Dockerfile` is present, build it as the base (new path); if absent, keep today's default-base + `image/` overlay + `setup-scripts` path exactly as-is. Purely additive — nothing removed yet.
3. **Hardening rewire + overridable→base + at-task inject + `env.d` — additive/transitional.** *(Security boundary; interactive/integration.)* Move the overridable defaults into `cove-base-image` and delete `internal/assemble/overridable` (hardening COPYs sealed-only; preserve the `.claude.json` install-merge from the in-image path); inject the embedded at-task; add the `/etc/cove/env.d/*.env` fold (base ships `00-base.env`). This requires **republishing `cove-base-image`** (defaults + `00-base.env`) → a new blessed digest appended per COV-37, and pointing at-cove's default base at it. The `cove-base-image` content edit is folded in here (one coherent behavior change), not a separate COV-33 ticket. **The legacy `image/` overlay + `setup-scripts` fallback is left working** — this PR removes nothing kits depend on. Real build validates *both* a `image/Dockerfile` kit and a legacy `setup-scripts` kit.
4. **Migrate the two kits, then retire the legacy path.** *(Interactive/integration.)* Convert `.at-cove` + `kits/reference-worker` from `setup-scripts`/`env`/`paths` to an `image/Dockerfile` (+ an `/etc/cove/env.d/` fragment). *Then*, with no kit left on it, remove the legacy path: delete `run-setup.sh`, the kit-`image/` overlay + collision machinery, and the old `apply-image-env.sh` config path; drop `setup-scripts`/`env`/`paths` (and the `baseEnvKeys` guard) from the schema. End-state: `image/` is *only* the kit Dockerfile's build context. Verify both migrated kits build to a working hardened image.
