...
3. otherwise **unresolved** — what happens next depends on whether **S** is
   *required*:
   ...
   - on the `work`/`dispatch` path specifically, the agent's Anthropic
     ...
     `ANTHROPIC_API_KEY` — is required the same way (declared with no supply,
     or not declared at all) — a keyless worker is a guaranteed 401, so
