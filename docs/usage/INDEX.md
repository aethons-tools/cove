...
| [at-cove-config.md](at-cove-config.md) | The `at-cove` kit `config.yml` schema — every field (name, backend, setup, secrets, loops, image, dispatch), validation rules, and a full annotated example. | You are authoring or editing a kit's `.at-cove/config.yml`. |
