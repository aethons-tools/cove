...
| [harbor control-plane MVP](../superpowers/specs/2026-09-11-harbor-control-plane-mvp-design.md) | Slice #2: `at-harbor serve` adds a loopback admin API alongside the broker, both backed by one live file store; `enroll`/`revoke`/`destination add\|list\|rm\|import` become admin-API clients (`--admin-url`), so destinations and enrollments change at runtime with no restart. `harbor.yaml` is bootstrap-only (`listen`, `admin-listen`, `tls`, `store`, `credentials`); credentials stay config-sourced (validated, never stored). | You are managing a running harbor's destinations or enrollments at runtime, or writing its bootstrap serve config with `admin-listen`. |
