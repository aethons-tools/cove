...
| [harbor/](harbor/INDEX.md) | Operating the `at-harbor` central service — running it (`serve`, the broker, destinations, TLS), signing operators in (OIDC, `login`/`whoami`, `--app` profiles), the roster/RBAC (`role`/`grant`/`roster`/`enroll`), and the kit registry (`kit`). A section index routing to the four operator docs. | You are running or administering a harbor — standing it up, signing in, deciding who can reach what, or registering/versioning kits. |
