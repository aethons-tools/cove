...
- **Endpoint:** harbor serves `/squawks` on its cove-facing `:443` mux. A request carries the cove's identity token (`Authorization: Bearer`); harbor resolves the actor, derives **that actor's own ticket** (`Instance.Unit`), and calls Linear.
...
A cove's inbox is a **durable, acked queue** over the squawk Log, not a snapshot
...
- **Commit cursor.** Each cove has a durable commit cursor (its last *processed*
  squawk id) stored on its instance in the harbor store. It is **initialized at
  raise to the Log's current tail**, so a freshly-raised cove consumes squawks
...
- **Explicit commit.** When the cove has durably handled squawks, it calls
...
external-origin squawk addressed to the cove (append `seq` > `WaitSeq`) counts as a
...
by the relay ingress engine above, so **without a configured `intercom-log:`, a
