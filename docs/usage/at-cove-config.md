...
story, including the auth prerequisite (the first `at-cove teammate <class>` run
...
`at-cove chat` resolves against both maps too, but rejects a teammate class with
a usage error pointing at `at-cove teammate`.
