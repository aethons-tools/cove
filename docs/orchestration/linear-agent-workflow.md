...
On success it writes its artifacts (branch and PR URL) to `task-result.json`; the scheduler reads the result and brokers the tracker update — post artifacts, and move `IN PROGRESS → IN REVIEW` (or `→ DONE` for stages with no review).
...
1. **Stops.**
2. Emits a **structured `needs-input` result** (the worker-result's `needs-input` block, echoed into `task-result.json`) using a fixed template:
...
- **Stale-claim reaper:** on each poll pass the scheduler reaps `IN PROGRESS` issues that are **in a dispatchable worker class**, whose time-in-state exceeds `dispatch.reaper-timeout`, and that **no live in-process dispatch owns**, moving each to `NEEDS INPUT` with an explanatory comment. The worker-class scope is essential: the reaper only touches issues the dispatcher itself could have claimed — an interactive (`class:planner`), unknown-class, or unlabeled `IN PROGRESS` issue is a human's to manage and is left alone (it was never the scheduler's claim to reap). This is orthogonal to the per-dispatch time budget (which bounds a *running* worker): the reaper backstops the case where the in-process dispatch is gone — a crashed or hung worker, or a scheduler restart mid-run — but the tracker still shows `IN PROGRESS`. A run the scheduler is still actively dispatching is never reaped, however long it takes.
