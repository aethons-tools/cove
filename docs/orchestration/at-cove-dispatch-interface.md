...
3. **Fill + inject the task** — at-cove parses `--in`, **fills the target repo** into it from the kit's [`origin`](../usage/at-cove-config.md) + `main-branch` (the scheduler names no repo — the kit is the single source), and writes the completed task over SSH stdin to the at-cove-owned VM path `/home/agent/work/.at-task/task.json`.
...
The worker's contract is **at-task's**, not a bespoke `result.json`. The scheduler builds the task file (naming **no repo**); at-cove fills the target repo from the kit's `origin`, injects the task at `.at-task/task.json`, and extracts the result from `.at-task/task-result.json`, both fixed, at-cove-owned VM paths (no kit-declared input/output). The file shapes, the JSON Schemas, and the `.at-task/` file-handoff convention (`task.json` → `worker-result.json` → `task-result.json`) are owned by the [at-task usage doc](../usage/at-task.md) and its linked [inputs](../usage/at-task-inputs.md)/[output](../usage/at-task-output.md) contract docs — not restated here.
