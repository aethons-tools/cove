...
- **Shipped:** `at-cove work` (synchronous one-shot, ephemeral container, crash-scavenge); the reference worker kit and the host-orchestrated bracket at-cove drives (`at-task prepare` → agent → `at-task complete`) with the per-step credential air-gap; **per-class session egress applied before the agent step** (the `<common> ∪ class` delta from `install.json`, via `ApplySessionEgress` — [COV-39 §5](../superpowers/specs/2026-07-19-per-class-egress-design.md), ephemeral path); at-task's `.at-task/task.json` → `task-result.json` contract; the scheduler wiring ([at-cove-config.md](../usage/at-cove-config.md)); container-per-task isolation; the `COVE_RUN_*` run-parameter passthrough and the resulting per-run token minter (`at-mint github`), minted fresh before each git step.
- **Deferred:** the persistent (`chat`) half of per-class egress (COV-39 S5); multi-code-host (GitHub-only today).
