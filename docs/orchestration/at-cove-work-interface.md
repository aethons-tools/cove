---
summary: The substrate by which a dedicated scheduler dispatches one-shot worker agents through at-cove — the shipped `at-cove work` command (synchronous, one-shot, ephemeral hardened container from a kit), at-cove's host-orchestrated worker bracket (`at-task prepare` → agent → `at-task complete`) and the per-step credential air-gap it enforces, the per-class session egress it scopes before the agent step, the `.at-task/task.json` → `task-result.json` worker contract at at-cove-owned VM paths, the three-authority credential model, per-class isolation, and the `COVE_RUN_*` passthrough that turns a secret resolver into a per-run token minter.
read_when: You are implementing or reviewing how the scheduler launches workers on hardened at-cove/Colima containers — the `at-cove work` command it calls, how the input/output handoff works, how at-cove drives the prepare/agent/complete bracket, air-gaps the worker from the code-host token, and scopes the worker class's egress before the agent step, or the credential model (the `COVE_RUN_*`-driven per-run minter).
owns: the at-cove work substrate contract, at-cove's host-orchestrated worker bracket + credential air-gap + per-class session-egress scoping (when applied), the worker input/output handoff pointer, the three-authority credential model, and per-class isolation
...
5. **Scope egress to the worker class** — once the booted container is reachable and **before any bracket step runs**, at-cove resolves the class's [`<common> ∪ class` domain delta](../usage/at-cove-config.md#workersclassallowed-domains) from the currency-pinned `install.json` run-config (`kit.Config.ResolvedWorkerDomains` — never a live `config.yml`) and applies it via the backend's `ApplySessionEgress` (a privileged host `docker exec` of the sealed `apply-session-domains.sh` + `squid -k reconfigure`; see [OVERVIEW](../OVERVIEW.md#backends)). The container booted with the baked sealed + root lists; this adds only the class's delta for this one unit, so the agent workload never sees a wider-than-intended egress window. A class with no extra domains applies an empty delta (root still applies). This is the ephemeral (`work`/`dispatch`) half of [COV-39 §5](../superpowers/specs/2026-07-19-per-class-egress-design.md); the persistent (`chat`) half applies the collaborator's delta on session start and clears it on exit (see [OVERVIEW](../OVERVIEW.md#the-chat-command-and-collaborator-sessions)).
6. **Drive the worker bracket itself, step-by-step over ssh**: `at-task prepare` (env **with** `AT_TASK_GIT_TOKEN`) → `claude -p "<class prompt + result protocol>"` (env **without** the token) → `at-task complete` (env **with** the token), each bounded by `--timeout` — which, when omitted, defaults to the resolved [`workers.<class>.timeout`](../usage/at-cove-config.md#workers) (the same cap the dispatch scheduler passes, so a manual `work` matches `dispatch`), an explicit flag always winning (COV-88). at-cove reads `worker.class` from the task to resolve the kit's `workers[class].prompt` — so the task is not opaque (at-cove reads the class and fills the repo), but its brief and other contents are. Each step's SSH channel is **captured, not spilled**: `at-task`'s structured JSONL (stderr) is merged into the host log stream with run/issue/class/step context, while the agent's raw output stays VM-local — see [VM output capture, demux, and merge](../OVERVIEW.md#command-surface).
7. **Extract** the at-cove-owned VM path `/home/agent/work/.at-task/task-result.json` to `--out`; **destroy** the container on every exit path.
...
**PR merge auto-closes the issue (github tracker, same-repo).** Reaching Done is normally the scheduler's/reviewer's write, but for a **GitHub Issues** tracker the merge itself can retire the issue: the scheduler stamps the optional [`issue.closes`](../usage/at-task-inputs.md) field with the issue's `#N` reference, and `at-task complete` appends one `Closes <ref>` line to the PR body — so merging the worker's PR closes the issue (= Done). The scheduler sets it **only** when the active tracker is `github` **and** the issue's repo (`tracker.github.repo`, defaulting to `source-control`) equals the PR's target repo (`source-control`); for a **Linear** tracker or a **cross-repo** GitHub issue the field stays empty and no line is appended. This keeps `at-task` **tracker-agnostic** — it forwards the field, never knowing which tracker (if any) computed it — and does not break the single-writer model: auto-close and a reviewer-driven `Done` transition are both idempotent (closing a closed issue is a no-op), so the two never conflict.
...
- **Shipped:** `at-cove work` (synchronous one-shot, ephemeral container, crash-scavenge); the reference worker kit and the host-orchestrated bracket at-cove drives (`at-task prepare` → agent → `at-task complete`) with the per-step credential air-gap; **per-class session egress** — the `<common> ∪ class` delta from `install.json` applied via `ApplySessionEgress` ([COV-39 §5](../superpowers/specs/2026-07-19-per-class-egress-design.md)), the ephemeral (`work`/`dispatch`) path applying the worker class's delta before the agent step and the persistent (`chat`) path applying the collaborator's on start + clearing it on exit; at-task's `.at-task/task.json` → `task-result.json` contract; the scheduler wiring ([at-cove-config.md](../usage/at-cove-config.md)); container-per-task isolation; the `COVE_RUN_*` run-parameter passthrough and the resulting per-run token minter (`at-mint github`), minted fresh before each git step.
- **Deferred:** multi-code-host (GitHub-only today).
