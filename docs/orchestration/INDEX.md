...
| [at-cove-dispatch-interface.md](at-cove-dispatch-interface.md) | The shipped substrate: the `at-cove dispatch` command, the kit-declared entrypoint + credential air-gap, at-work's `.at-work/` task.json → task-result.json worker contract, the three-authority credential model, and per-class isolation (the per-task minter is deferred). | You need the *how* — the concrete contract by which the scheduler launches workers on at-cove. |
