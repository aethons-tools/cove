...
| [at-cove-dispatch-interface.md](at-cove-dispatch-interface.md) | The shipped substrate: the `at-cove dispatch` command, at-cove's host-orchestrated worker bracket + credential air-gap, at-task's `.at-task/` task.json → task-result.json worker contract, the three-authority credential model, and per-class isolation (the per-task minter is deferred). | You need the *how* — the concrete contract by which the scheduler launches workers on at-cove. |
