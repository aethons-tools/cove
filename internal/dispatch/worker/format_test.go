...
func writeAtTask(t *testing.T, dir, file, body string) {
	t.Helper()
	if err := os.MkdirAll(filepath.Join(dir, ".at-task"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, ".at-task", file), []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
}
...
func TestResolveContract(t *testing.T) {
	...
	writeAtTask(t, dir, "task.json", "{}")
	p, ext, err := resolveContract(dir, "task")
	if err != nil || ext != ".json" || p != filepath.Join(dir, ".at-task", "task.json") {
		t.Fatalf("json: %q %q %v", p, ext, err)
	}
	writeAtTask(t, dir, "task.yml", "{}")
...
func TestDecodeStrictAndLenient(t *testing.T) {
	...
	writeAtTask(t, dir, "j.json", `{"name":"a","count":1,"extra":true}`)
	var s fmtSample
	if err := decodeFile(filepath.Join(dir, ".at-task", "j.json"), true, &s); err == nil {
		t.Fatal("strict JSON: unknown field must error")
	}
	if err := decodeFile(filepath.Join(dir, ".at-task", "j.json"), false, &s); err != nil || s.Name != "a" || s.Count != 1 {
		t.Fatalf("lenient JSON: %+v %v", s, err)
	}
	// YAML with an unknown field
	writeAtTask(t, dir, "y.yml", "name: b\ncount: 2\nextra: true\n")
	var s2 fmtSample
	if err := decodeFile(filepath.Join(dir, ".at-task", "y.yml"), true, &s2); err == nil {
		t.Fatal("strict YAML: unknown field must error")
	}
	if err := decodeFile(filepath.Join(dir, ".at-task", "y.yml"), false, &s2); err != nil || s2.Name != "b" {
