...
func TestDashboard(t *testing.T) {
	...
	rec := get(t, adminui.Handler(store, testLogger(), nil, anyCred), "/ui/")
...
func TestCovesFullPage(t *testing.T) {
	store := newStore(t)
	seedCove(t, store)
	rec := get(t, adminui.Handler(store, testLogger(), nil, anyCred), "/ui/coves")
	if rec.Code != http.StatusOK {
...
func TestCovesFragment(t *testing.T) {
	...
	adminui.Handler(store, testLogger(), nil, anyCred).ServeHTTP(rec, req)
	body := rec.Body.String()
...
func TestRosterView(t *testing.T) {
	...
	body := get(t, adminui.Handler(store, testLogger(), nil, anyCred), "/ui/roster").Body.String()
...
func TestRosterViewNoSecretLeak(t *testing.T) {
	...
	if strings.Contains(get(t, adminui.Handler(store, testLogger(), nil, anyCred), "/ui/roster").Body.String(), "HASH-NOPE") {
...
func TestRolesView(t *testing.T) {
	...
	body := get(t, adminui.Handler(store, testLogger(), nil, anyCred), "/ui/roles").Body.String()
...
func TestKitsView(t *testing.T) {
	...
	body := get(t, adminui.Handler(store, testLogger(), nil, anyCred), "/ui/kits").Body.String()
...
func TestDestinationsView(t *testing.T) {
	...
	body := get(t, adminui.Handler(store, testLogger(), nil, anyCred), "/ui/destinations").Body.String()
...
func TestStaticRouteDoesNotServeTemplates(t *testing.T) {
	h := adminui.Handler(newStore(t), testLogger(), nil, anyCred)
