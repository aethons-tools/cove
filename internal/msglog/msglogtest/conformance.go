...
func actor(r string) msglog.Target { return msglog.Target{Kind: "actor", Ref: r} }
func human(r string) msglog.Target { return msglog.Target{Kind: "human", Ref: r} }

// bodies extracts the Body of each message, for compact test failure output.
func bodies(ms []msglog.Message) []string {
...
func testMixedNamespaceOrdering(t *testing.T, newStore func(t *testing.T) msglog.Store) {
	...
	m1, err := s.Append(msglog.Message{ID: "in:linear:zzz", From: human("h"), To: []msglog.Target{x}, Body: "1"})
	if err != nil {
		t.Fatal(err)
	}
	m2, err := s.Append(msglog.Message{ID: "in:discord:aaa", From: human("h"), To: []msglog.Target{x}, Body: "2"}) // lexically < m1
	if err != nil {
		t.Fatal(err)
	}
	m3, err := s.Append(msglog.Message{From: human("h"), To: []msglog.Target{x}, Body: "3"}) // digit-prefixed internal id
	if err != nil {
		t.Fatal(err)
	}
	got := s.ReadInbox(x)
