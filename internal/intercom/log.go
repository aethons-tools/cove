...
func Open(path string, log *slog.Logger) (*Log, error) {
	...
	if data, err := os.Open(path); err == nil {
		...
		if err := sc.Err(); err != nil {
			log.Warn("intercom: scan ended early", "error", err.Error())
		}
	} else if !os.IsNotExist(err) {
		return nil, fmt.Errorf("intercom: open %s: %w", path, err)
	}
	var maxSeq int64
	...
	if err != nil {
		return nil, fmt.Errorf("intercom: open-append %s: %w", path, err)
	}
	l.f = f
	return l, nil
}
...
func (l *Log) Append(m Squawk) (Squawk, error) {
	...
	if err != nil {
		return Squawk{}, fmt.Errorf("intercom: marshal: %w", err)
	}
	if _, err := l.f.Write(append(line, '\n')); err != nil {
		return Squawk{}, fmt.Errorf("intercom: write: %w", err)
	}
	l.msgs = append(l.msgs, m)
	return m, nil
}
